...
- **گزارش بنرها در `/analytics` (جلسه ۴۸ — کامیت استیج، بلاک هوک):** کارت «گزارش بنرها (کارت بومی)» در `pages/analytics/index.vue` — دو جدول: (۱) به تفکیک جایگاه: نمایش/کلیک/CTR/لید/لید از هر کلیک (۲) به تفکیک متن بنر برای مقایسه A/B (تیتر/جایگاه/وضعیت/نمایش/کلیک/CTR). اکشن `getBannerSummary` در `store/admin/track/index.ts` → `GET /api/banner/track-summary?days=` (بک‌اند: `BannerAdminController::bannerSummary` — تجمیع track_events؛ لید فقط منسوب به بنر یعنی generate_lead با promotion_name؛ CTR/نرخ = null وقتی مخرج صفر → «—»). فیلتر بازه ۷/۳۰/۹۰ روی هر دو گزارش اثر میکند.
- ⚠️ **هوک Mimosa رفتار ناپایدار دارد:** کامیت قبلی ادمین عبور کرد، کامیت بعدی با یافته کهنه بک‌اند بلاک شد (فایل بیربط). کامیت به ترمینال کاربر سپرده شد.

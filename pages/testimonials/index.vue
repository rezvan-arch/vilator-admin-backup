...
export default {
  setup() {
    ...
    useHead({
      title: "پنل مدیریت | نظرات مشتریان",
    });
...
<template>
  <section class="testimonial__list">
    <div class="card">
      <div class="card__header">
        <h4 class="heading__title">لیست نظرات مشتریان</h4>
        <nuxt-link to="/testimonials/form/new" class="btn btn-primary">
            ...
            <table>
              ...
              <tbody>
                <tr v-for="(item, index) in store.data" :key="index">
                  ...
                  <td>{{ item.rating || "-" }}</td>
                  <td>
                    <span v-if="item.property" :title="item.property.title">
                      {{ item.property.title }}
                    </span>
                    <span v-else>-</span>
                  </td>
  ...
  <transition name="fade">
    <AdminModalsConfirmModal
      ...
      msg="آیا از حذف این نظر اطمینان دارید؟"

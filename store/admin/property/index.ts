  ...
  actions: {
    ...
    async updateProperty(id: string) {
      ...
      slugList.forEach((item) => {
        idsList[`${item}_id`] = null;

        if (this.properties[item] != null) {
          idsList[`${item}_id`] =
            this.properties[item].id != undefined
              ? this.properties[item].id
              : this.properties[item];
        }
      });

      let location_id: null | string = null;
      locSlugList.reverse().forEach((item) => {
        if (
          item != "highway" &&
          item != "downtown" &&
          item != "suburbs" &&
    ...
    async changePropertyPriceByPercentage(data: any) {
      return await this.$axios.post(`/api/property/change/percentage`, data);
    },

    ...
    deleteItem() {
      if (this.deleteID != "") {
        ...
        this.store.deleteSingle(this.deleteID).then((res) => {
          if (res.status == "success") {
            this.$toast("محتوای لندینگ حذف شد!", "success", 2000);
          } else {
...
<template>
  <section class="landing-content__list">
    <div class="card">
      <div class="card__header">
        <h4 class="heading__title">محتوای لندینگهای فروش (/property)</h4>
        <nuxt-link to="/landing-content/form/new" class="btn btn-primary">
            ...
            <table>
              ...
              <tbody>
                <tr v-for="(item, index) in store.data" :key="index">
                  <td class="font-mono text-xs" dir="ltr">{{ item.segment_key }}</td>
                  <td>{{ item.heading }}</td>
                  <td>{{ (item.intro || []).length }}</td>
                  <td>{{ (item.faqs || []).length }}</td>
                  <td>
                    <span
                      class="badge"
                  ...
                  <td class="setting">
                    <div class="actions justify-end">
                      <nuxt-link
                        :to="`/landing-content/form/${item.id}`"
                        class="edit__action"
                      >
  ...
  <transition name="fade">
    <AdminModalsConfirmModal
      ...
      msg="آیا از حذف این محتوا اطمینان دارید؟ (صفحه لندینگ به محتوای پیشفرض کد برمیگردد)"
      @confirm="deleteItem()"

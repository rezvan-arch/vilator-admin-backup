...
export default {
  setup() {
    ...
    store.notActiveLoading = true;
    store.actionUserId = "";
    store.notActivePagination = [];
    useHead({
      title: "پنل مدیریت | کاربران غیرفعال",
    });
    return {
      store,
    };
  ...
  created() {
    ...
    this.store.getUserNotActive(this.currentPage);
  },
  methods: {
    ...
    trashUser() {
      if (this.store.actionUserId != "") {
        ...
        this.store.TrashUser().then((res) => {
          ...
          this.store.notActiveUsers = [];
          this.store.notActiveLoading = true;
          this.store.getUserNotActive(this.currentPage);
        });
      }
    },
    pagination(page = 1) {
      if (this.currentPage != page) {
        ...
        this.store.getUserNotActive(page);
      }
    },
  },
};
    ...
    <section class="users__list">
      <div class="card">
        <div class="card__header">
          <h4 class="heading__title">لیست کاربران غیرفعال</h4>
        ...
        <div class="card__body table">
          <div v-if="!store.notActiveLoading">
            <div v-if="store.notActiveUsers.length > 0">
              <table>
                <thead>
                  <tr>
                ...
                <tbody>
                  <tr
                    v-for="(item, index) in store.notActiveUsers"
                    :key="index"
                  >
                    <td>
              ...
              <AdminPagination
                v-if="
                  store.notActivePagination &&
                  store.notActivePagination.last_page > 1
                "
                @onChange="pagination"
                :currentPage="currentPage"
                :lastPage="store.notActivePagination.last_page"
              />
            </div>

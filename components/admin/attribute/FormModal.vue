...
export default {
  ...
  setup() {
    const { attributeStore, adminStore } = useStore();
  ...
  methods: {
    addAttr() {
      ...
      this.attributeStore
        .addAttribute({ ...this.form, parent_id: this.id })
        .then((res) => {
...
<template>
  <div @click.self="$emit('closeModal')" class="form__modal">
    <div class="modal__base">
      <div class="modal__header">
        <h2 v-if="editModal">ویرایش آپشن</h2>
        <h2 v-else>اضافه کردن آپشن</h2>
      ...
      <div class="modal__body">
        <vForm v-slot="{ errors, handleSubmit }" as="form">
          <form @submit.prevent="handleSubmit(editAttr)">
            <div class="form">
              <div class="row">

...
function pagination(page = 1) {
  if (currentPage.value != page) {
    ...
    propertyStore.getProperty(page);
  }
}

function trashPropertyConfirm(propertyId) {
  propertyStore.actionPropertyId = propertyId;
  trashConfirm.value = true;
}

function trashProperty() {
  if (propertyStore.actionPropertyId != "") {
    $toast("منتظر بمانید...", "info", 2000);
    trashConfirm.value = false;
    propertyStore.trashProperty().then((res) => {
      propertyStore.actionPropertyId = "";
...
function searchCity(data) {
  locationStore.getLocationsSelect(
    data,
    "city",
    propertyStore.properties.province.value
  );
}
...
function searchSuburb(data) {
  locationStore.getLocationsSelect(
    data,
    "suburb",
    propertyStore.properties.city.value
  );
}
...
const selectFn = () => {
  selected.value = [];
  if (!selectAll.value) {
    propertyStore.properties.forEach((item) => {
...
watch(selected, () => {
  selectAll.value =
    selected.value.length == propertyStore.properties.length ? true : false;
});
...
function setReqBody() {
  let head = Object.fromEntries(
    Object.entries(propertyStore.filterHead).filter(
      ([_, v]) => v != "" || v.length != 0
    )
  );

  propertyStore.requestBody = { ...head };

  propertyStore.staticFilter.forEach((item) => {
    if (item.value != "") {
      propertyStore.requestBody[item.slug] =
  ...
  propertyStore.filtersData.forEach((item) => {
    if (item.value != "") {
      propertyStore.requestBody[item.slug] =
        item.options.mode == "string"
          ? String(item.value)
...
const propertyCode = ref(null);
function submitPropertyCode(){
  propertyStore.staticFilter.forEach(item=>{
    if(item.slug === 'property_code'){
      item.value = propertyCode.value;
    }
  })
  submitFilter();
}
  ...
  <section class="properties__list">
      ...
      <div class="card__body collapse__body" ref="filterBody">
        <div class="row">
          <div class="controls w-1/2">
            ...
            <v-select
              v-model="propertyStore.filterHead.type"
              id="owner"
              :options="typeList"
              multiple
              :closeOnSelect="false"
        ...
        <div class="row">
          <div class="controls w-1/4">
            <label for="owner">وضعیت ملک</label>
            <v-select
              v-model="propertyStore.filterHead.property_status"
              id="owner"
          ...
          <div class="controls w-1/4">
            ...
            <v-select
              v-model="propertyStore.filterHead.city"
              :reduce="(option) => option.slug"
              id="city"
          ...
          <div class="controls w-1/4">
            ...
            <v-select
              v-model="propertyStore.filterHead.suburb"
              :reduce="(option) => option.slug"
              id="suburb"
              ...
              :disabled="
                ...
                propertyStore.filterHead.city == null
              "
              autocomplete="off"
            >
              <template #no-options>
          ...
          <div class="controls w-1/4">
            ...
            <v-select
              v-model="propertyStore.filterHead.district"
              :reduce="(option) => option.slug"
              id="district"
        ...
        <div class="row flex-wrap">
          <div
            v-for="(item, index) in propertyStore.staticFilter"
            :key="index"
          ...
          >
            ...
            <template v-else>
              ...
              <input
                ...
                :id="item.item"
                class="form-control"
                :class="[{ 'text-left': item.slug == 'property_code' }]"
                :style="{
                  direction: item.slug === 'property_code' ? 'ltr' : 'rtl',
                }"
                @input="setReqBody"
              />
            </template>
          ...
          <div
            v-for="(item, index) in propertyStore.getFilteredData"
            :key="index"
            class="controls w-1/4 mb-5"
          >
    ...
    <div class="card">
      <div class="card__header">
        <h4 class="heading__title">لیست ملک ها</h4>
        <div class="heading__actions">
          <div class="search__property__code">
            <form @submit.prevent="submitPropertyCode">
              <input
                v-model="propertyCode"
                type="text"
                placeholder="کد ملک"
                class="border !rounded-r-full border-l-0 outline-none px-4 h-[46px] placeholder:text-right text-left"
                style="direction: ltr"
              />
              <button class="btn btn-primary !rounded-l-full !px-4">
                جستجو
              </button>
            </form>
          </div>
          <nuxt-link to="/property/new" class="btn btn-primary">
            <i class="fa-regular fa-plus"></i>
            ملک جدید
          </nuxt-link>
          <nuxt-link to="/property/trash" class="btn btn-danger">
      ...
      <div class="card__body table">
        <div v-if="!propertyStore.loading">
          ...
          <div v-if="propertyStore.properties.length > 0">
            ...
            <AdminPagination
              v-if="
                propertyStore.pagination &&
                propertyStore.pagination.last_page > 1
              "
              @onChange="pagination"
              :currentPage="currentPage"
              :lastPage="propertyStore.pagination.last_page"
            />
          </div>
